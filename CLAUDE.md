...
**LLM Gateway:** All LLM calls (account summaries, per-ticket summaries, QBR insights, roadmap selection) route through the LangSmith LLM Gateway (`gateway.smith.langchain.com`). The gateway resolves provider API keys from your LangSmith workspace secrets, so no provider keys (Anthropic, OpenAI, Google) are needed locally. `start.sh` sets the per-provider `*_BASE_URL` and `*_API_KEY` env vars from the gateway key at startup. `llm.py` checks `LLM_GATEWAY_API_KEY` first (for LSD), then `LANGSMITH_API_KEY` and `ANTHROPIC_API_KEY` (for local dev). The model registry in `backend/llm.py` defines which models are available; the frontend fetches them via `GET /api/models`. `DEFAULT_MODEL_ID` is `anthropic:claude-sonnet-5` and must stay the first entry (the frontend preselects it; an assert enforces this). It's also the model for scheduled reports without a saved model. `get_chat_model` accepts any `provider:model` for the five gateway providers, so schedules saved with a model later dropped from the list keep working. IDs the provider has actually retired go in `_RETIRED_MODELS` (old ID → replacement, remapped with a warning). OpenAI models use the Responses API (`use_responses_api=True`): GPT-6 only accepts tools alongside reasoning there. Anthropic models get `max_tokens=16000`: the langchain-anthropic default of 4096 can be used up entirely by Claude's built-in thinking, leaving an empty reply (`generate_account_summary` raises rather than return or cache an empty summary). Only listed models are cached. Before changing the list, run `cd backend && uv run python check_models.py`: it sends each model a short prompt, a tool call and a `temperature=0` call through the gateway, and exits non-zero on any failure, including when `llm._NO_TEMPERATURE` doesn't match what the model accepts. Providers retire models without notice, and model listings can include models that aren't deployed.
...
- `GET /api/accounts?tier=Premium` — sorted list of accounts for the given tier (defaults to Premium); grouped accounts are replaced by their group entry (`{id: "grp_…", name, members: [{id, name}]}`), listed under every tier any member is in
- `GET /api/account-groups` / `GET /api/account-groups/candidates` / `POST /api/account-groups` (body `{name, member_ids}`, 2–10 current-customer accounts not already grouped) / `PATCH /api/account-groups/{group_id}` (partial body `{member_colors?: {member_id: slot 1–8}, member_labels?: {member_id: label ≤40 chars, "" = reset}}`; any colour combination allowed) / `DELETE /api/account-groups/{group_id}` (ungroup; 409 while any schedule targets the group) — see `account_groups.py`
- `GET /api/accounts/{id}/data?account_name=...&period=` — metrics + open issues, 5-minute in-memory TTL cache. Payloads carry `version` (`_PAYLOAD_VERSION`); bump it whenever the payload or normalised-issue shape changes so cached payloads (memory + 8h disk) are rebuilt instead of served stale. Each issue's `tags` is a single category breadcrumb from `_category_path`: the Pylon category label (split on " - ") plus the free-text `category_component` path (e.g. `agent_builder/inbox/notifications`, no Pylon labels) humanised segment by segment, old names mapped via `_RENAMED` ("Agent Builder" → "Fleet"), and segments repeating the path dropped → `["Fleet", "Inbox", "Notifications"]`
- `GET` / `PATCH /api/accounts/{id}/settings` — per-account dashboard settings `{show_linked_ids: bool (default false), flag_duplicates: bool (default true)}`, shown as sidebar switches. Stored in the LangGraph Store (`account_settings.py`, namespace `psh_account_settings`, keyed by Pylon account id or group id), so turning it on in the sidebar keeps it on for that account, for everyone. 404 for unknown accounts
- **Linked ticket IDs**: normalised `external_issues[]` carry `display_id` from `_external_display_id` — Linear's Pylon `external_id` is a UUID, so the key (`LSO-4456`) is parsed from the link; GitHub `owner/repo/123` → `repo#123`. Exports take their own opt-in flag, off by default and independent of the sidebar setting: `/report?linked_ids=true`, `show_linked_ids` on the email/Slack/schedule bodies (stored in the cron payload; older schedules without it = off). For Slack, the flag rides in the interactive button value (`"linked": true`) so the "Current Open Issues" list (and "Show Next") includes `:link: \`ID\``. In exports/shares the IDs are plain text with no source name and no links (customers can't open our Linear/GitHub); only the internal dashboard links them. UI: sidebar switch, the `ExportOptions` checkboxes in DownloadMenu/ShareButton/ScheduleModal, CSV "Linked tickets" column
- **Possible duplicates** (`duplicates.py`): `GET /api/accounts/{id}/duplicates?model=` → `{groups: [{id, tickets, reasons: [{kind: link|ai|manual, text, tickets}]}], dismissed: [{id, tickets, by, at}], ai_status: done|pending|failed}`; `POST .../duplicates/analyze {model}` runs the AI pass; `POST .../duplicates/dismiss {tickets}`, `.../restore {dismissal_id}`, `.../mark {tickets: [a, b]}`. Groups are connected components over edges from (a) tickets sharing a linked Linear/GitHub issue (keyed on source + `external_id`, so links whose readable `display_id` can't be parsed still group) — always flagged, (b) one LLM call over each open ticket's title, category breadcrumb and cached summary (cached on disk per model under `dup:{account_id}:{model}`, keyed by a hash of model + that text, so it reruns only when tickets/summaries change; one run per account at a time; model output restricted to the account's open ticket numbers), (c) manual marks. The AI call runs at temperature 0 (that call only; models listed in `llm._NO_TEMPERATURE` (Sonnet 5, Opus 5.5, Fable 5.1, GPT-6), which reject a temperature, run at their default via `llm.supports_temperature`; any other model that rejects it retries at its default and is remembered per process in `duplicates._NO_TEMPERATURE`), and AI groups are sticky: each successful run is merged into `ai_memory` in the account's duplicates state (same ticket set → reason updated, closed tickets pruned), and `build()` keeps remembered groups a later run doesn't repeat unless dismissed — so groups don't flicker when summaries are reworded. A run is only remembered when every ticket had a cached summary, and the frontend only triggers `analyze` once the *selected* account's summary pipeline has finished (`summariesDoneFor`), so passes over incomplete input can't become sticky; duplicates calls use the model those summaries were made with (`dupesModel`), since switching model doesn't re-run the summaries. `_duplicates_response` loads the cache file once per request (`cache.snapshot()`). AI reasons are one plain sentence of 15 words at most (dashes replaced via `text_style.replace_dashes`), the prompt is part of the AI cache key so editing it reruns the AI once, and an AI reason is dropped when a link or manual reason covers exactly the same tickets (no pair listed twice). Each reason carries the `tickets` it covers; in 3+ ticket groups a reason covering only some of them is prefixed with those tickets ("#23951 + #26255 · …") in the dashboard and exports. Dismissals are stored per ticket pair and drop link/AI edges between those tickets (a new ticket matching them brings the group back); manual marks ignore dismissals. A dismissal keeps the manual marks it removed so Restore puts them back. All state changes go through `duplicates._updating` (per-account lock + re-read). `GET …/duplicates?stale=true` serves the latest saved AI result without running the AI (used by the CSV export). Manual marks + dismissals live in the LangGraph Store (`psh_duplicates`, per account id). All mutations validate tickets are the account's current open tickets. The frontend loads groups with the account data and triggers `analyze` once the ticket-summary pipeline has settled; groups render as one dashed container (`DuplicateGroupCard`) at the first visible member's position, with a "Possible duplicates" toggle in the ticket toolbar, "N dismissed" restore popover (`DismissedDuplicates`) and a hover "Mark as duplicate…" action on `TicketCard`. Only when the account's `flag_duplicates` setting is on (off = no duplicate fetch/AI pass, plain ticket list, no mark action). Exports/shares have their own opt-in (off by default, independent of the sidebar setting): `/report?duplicates=true`, `show_duplicates` on the email/Slack/schedule bodies (Slack carries `"dupes": true` in the button value). `_export_duplicates` computes groups from the latest cached AI result even if tickets/summaries changed since (`allow_stale`), so downloads don't block ~30-60s on a fresh LLM pass while summaries settle; it only runs the AI when the account has never been analysed and `_export_duplicate_groups` returns reasons as `{tickets, text}` (tickets only for partial coverage; `report._render_duplicate_reasons` shows them as a non-wrapping left column) and rewrites them for customers — link reasons become "Linked to the same engineering ticket" (ID only when linked IDs are also on), manual ones "Marked as duplicates by our support team" (no staff emails). PDF/email render a tinted "POSSIBLE DUPLICATES" header row + members with a blue left edge at the first member's position; Slack reorders so group members are adjacent (pages are fixed slices of that order, recomputed per "Show Next" click — consistent unless the AI result changes between clicks) and adds "Possible duplicate of #…"; CSV adds a "Possible duplicate of" column. The export option checkboxes live in `components/ExportOptions.tsx` (`ExportOptions` {linkedIds, duplicates} passed through DownloadMenu/ShareButton/ScheduleModal)
...
- `POST /api/schedules` — body `{account_id, account_name, label, destination_type, channel_id?, email_addresses?, qbr_notify_type?, qbr_notify_channel_id?, qbr_notify_emails?, qbr_template_type?, sections?, period, model, frequency, weekday, nth, month_in_quarter, hour_local, timezone}`; `model` is the selected LLM model ID (stored in the cron payload so scheduled runs use the same model); `destination_type` is `"slack" | "email" | "qbr"` (Literal); QBR schedules ignore `sections`/`period` and require `qbr_notify_type` (`"slack"` or `"email"`), `qbr_notify_channel_id` (Slack), or `qbr_notify_emails` (email, must be `@langchain.dev`); `qbr_template_type` is `"full_deck"` (default) or `"support_highlights"` — selects which Google Slides template to use; all other fields Pydantic-validated; timezone passed natively to `crons.create(timezone=)`; `created_by` is the authenticated user's email, stored in the cron payload
- `DELETE /api/schedules/{cron_id}` — delete a scheduled report by LangGraph cron ID
- `POST /api/accounts/{id}/qbr-slides` — body `{account_name, template_type?, model?}`; streams SSE progress events while generating a QBR Google Slides deck; `template_type` is `"full_deck"` (default) or `"support_highlights"` — selects the template and skips BigQuery chart steps for `support_highlights`; `model` is the selected LLM model ID for AI insights and roadmap selection; fetches the last 4 months of roadmap items from Google Drive, deduplicates by title (latest month wins), uses the selected model to select the 3 most relevant items matching the account's open feature requests, inserts them into a template slide deck, saves the result to disk cache, and sends a final `result` event with `{url, month, month_label}`; also generates BigQuery-powered charts (commit usage with KPI tiles, usage composites, maturity radar, Academy sign-ups table) and inserts them into the deck (full deck only)
...
**`account_groups.py`** — Account groups: several Pylon accounts (e.g. Orange Group + Orange Business) shown as one dashboard account with a synthetic `grp_<hex>` ID. Stored in the LangGraph Platform Store (namespace `psh_account_groups`, via `lg_store.py`, 30s in-process cache; `create_group` re-checks member/name clashes and `update_members` re-reads, both under a lock, before writing). `describe_members` is the single place member display rules (colour slot, custom vs default label) are resolved, for the API (`main._format_group`) and exports (`member_labels`). `pylon_client.search_issues_for_account`, `get_csat_responses_for_account` and `get_account` accept a group ID and expand it via `member_ids()` (issue searches run per member, merged, deduped and re-sorted newest first; an unknown `grp_` ID forces a fresh Store read before being treated as empty), so every account-scoped route, schedule and QBR works on groups unchanged. `get_account` returns a synthetic account: union of member Slack channels (first member's primary stays primary) and `custom_fields` from the first member with a Metronome ID. Ungrouping deletes the group; the member accounts reappear on their own, and group-keyed caches/QBR history are simply orphaned. Frontend: "Manage account groups" in the `AccountPicker` dropdown opens `AccountGroupsModal`. Normalised issues carry their own `account_id`; in a group view, `lib/accountLabels.memberLabels` combines each member's label (custom `label`, else the backend-derived `default_label` from `account_groups.default_member_labels` — shared leading words stripped, e.g. "Group"/"Business"; the only implementation) with a dot colour, shown as a pill on each `TicketCard` and as account filter chips. Dot colours are slots into the `--member-1..8` palette in `globals.css` (dataviz categorical palette; light values darkened to >= 3:1 on the pill surface), chosen per member in the Account Groups menu (stored as `member_colors`, default = slot order). Any combination is allowed, including similar or repeated colours — that's left to the user. Labels are editable per member in the Account Groups menu (stored as `member_label_overrides`; saving the default or an empty value clears the override so it keeps tracking the derived default); members in API responses carry `label` = the override or null. Exports carry the labels too (user-supplied text: escaped via `_e()` in HTML and `_slack_escape()` in Slack mrkdwn): `main._member_labels(account_id)` (`account_groups.member_labels`, looked up at render time so colour changes apply immediately; call it via `asyncio.to_thread` — it does a blocking Store request to this same server) feeds `generate_report_html(member_labels=…)` for PDF/email (coloured `&#9679;` + label, light-mode hex from `account_groups.MEMBER_COLOR_HEX`) and `_build_issues_blocks(member_labels=…)` for Slack (bold label prefix, text only); the CSV adds an Account column (full name). Pylon `logo_url`s were deliberately not used: sibling accounts usually share branding and the logos are illegible at badge size.
...
- `attach_requester_names(issues)` — Pylon issues only carry `requester: {id}`; resolves names via `GET /contacts/{id}` (falling back to the `/users` team list for staff-raised tickets), 4 parallel lookups, disk-cached in `.cache/contacts.json` for 7 days, and sets `issue["requester_name"]` in place (name only, never email). Called off the event loop from `main._fetch_raw_data` and the report dispatcher; normalised issues carry `requester_name`, shown as "Created … by NAME" on ticket cards, "Raised by" in PDF/email, 👤 in Slack, and a CSV Requester column
...
**`summary_agent.py`** — AI summary generation via `deepagents`. `generate_account_summary(...)` formats metrics as compact text and runs the agent. The account summary is customer-facing: two short plain-English paragraphs (150 words at most), warm and leaning positive without sugar-coating real problems (long-open Sev 1/2 bugs are named plainly with what we are doing about them), never blaming the customer, speaking to the customer as "you" (they raise tickets; LangChain closes them and owns the response time), themes rather than ticket lists, no bullets or headers. `_tidy_summary` enforces what models sometimes ignore: it strips preambles and dividers and replaces em/en dashes (`text_style.replace_dashes`, shared with duplicate reasons). When you change `SUMMARY_SYSTEM_PROMPT`, bump `cache.ACCOUNT_SUMMARY_VERSION` so cached summaries regenerate. `make_summarise_tickets_tool(open_issues, force, account_name)` returns a tool that generates and caches per-ticket summaries in parallel, passing the account name through to the prompt. `generate_qbr_insights(...)` generates `observations` and `opportunities` bullets for the Enterprise Support slide. `generate_usage_insights(...)` generates per-slide summaries, observations, and opportunities for the 3 LangSmith Usage slides (22-24) plus an Engagement Scorecard rollup, all in a single Claude Haiku call so everything stays aligned: slide 22 (commit usage — contract pacing, % commit used), slide 23 (tracing — traces, agent runs, page views, evaluator totals), slide 24 (feature adoption — experiments, Prompt Hub, datasets, evaluator rules by type). Returns `commit_summary`/`tracing_summary`/`feature_summary` (one-sentence per-slide headlines), `usage_summary` (rollup sentence for the Engagement Scorecard, synthesising all 3), and `commit_observations/opportunities`, `tracing_observations/opportunities`, `feature_observations/opportunities` (bullet lists).
...
**`cache.py`** — JSON file cache (`.cache/analysis_cache.json`). Writers are serialised by a lock and write atomically (temp file + `os.replace`), so a reader never sees a half-written file; `get_ticket_summaries` does a bulk lookup with one file load (used by `main._read_cached_summaries`, the single cached-summary reader for routes, Slack and the dispatcher). Per-ticket summaries keyed by `sha256(issue_id:latest_message_time)`; account summaries keyed by `as:{account_id}:{period}` (entries from an older `ACCOUNT_SUMMARY_VERSION` count as a miss); QBR slide records keyed by `qbr:{account_id}:{YYYY-MM}` (no expiry — permanent).
...
**`lg_store.py`** — Shared sync LangGraph Store helpers (`get`/`put`/`delete`/`search`, 404-tolerant, per-item `lock`) for `account_groups`, `account_settings` and `duplicates`. Call from worker threads only (`asyncio.to_thread` from async code): the Store is served by this same server, so a blocking call on the event loop deadlocks it.
...
│   ├── text_style.py           # Clean-up for AI-written customer-facing text (dash replacement)
│   ├── check_models.py         # Utility: test every model in llm.AVAILABLE_MODELS (prompt + tool call)
